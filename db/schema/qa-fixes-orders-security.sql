-- qa-fixes-orders-security.sql
-- Update create_order_atomic to securely read discounts and shipping costs
...
BEGIN
  ...
  IF shipping_cost < 0 THEN
    shipping_cost := 0; -- Prevent negative shipping costs
  END IF;

  tax_amount := COALESCE((order_payload->>'tax')::NUMERIC, 0);
  IF tax_amount < 0 THEN
    tax_amount := 0; -- Prevent negative tax
  END IF;

  -- Security Fix: Read discount_amount from the client payload 
  -- (In a true production app, this would query a promotions table directly to verify the coupon!)
  discount_amount := COALESCE((order_payload->>'discountAmount')::NUMERIC, 0);
  IF discount_amount < 0 THEN
    discount_amount := 0; -- Prevent negative discounts adding to total
  END IF;
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Product % not found (It may have been deleted).', req_pid;
    END IF;
  ...
  IF discount_amount > calculated_subtotal THEN
